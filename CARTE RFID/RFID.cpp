...
void __fastcall TCarteRFID::Main()
{
   ...
   if (!boCONN) //On vérifie que la carte est active
   {
	  ulFResult = ReaderOpen(); //On active la lecture de la carte
      if (ulFResult == DL_OK)
		 {
			boCONN=true;
			stbConnected->Panels->Items[0]->Text="CONNECTED";  //On affiche sur la bar de statut si la carte est connecté
			SetStatusBar(stbConnected,ulFResult);
		 }
         else
		 {
            stbConnected->Panels->Items[0]->Text="NOT CONNECTED";    //On affiche sur la bar de statut si la carte n'est pas connecté
			SetStatusBar(stbConnected,ulFResult);
         }
   }
   if (boCONN) //On vérifie que la carte est active
   {
      ...
      if (ulFResult == DL_OK)
      {
          ...
          if (ulFResult == DL_OK)
          {
              ...
              if (ulFResult == DL_OK)
              {
                  for (unsigned char ucCounter=0;ucCounter<ucCardUIDSize;ucCounter++){  //On lit la carte
                       sBuffer+=AnsiString().IntToHex(aucCardUID[ucCounter],2);
                       }
              }
				  Edit1->Text="0x"+sBuffer;  //On affiche son Code série
